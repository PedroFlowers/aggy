...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
		if isPremium(cid) then
			...
			selfSay('Hello ' .. creatureGetName(cid) .. '! I can Enter on Templar?( enter )( yes )')
			focus = cid
			talk_start = os.clock()
		else
  	...
  	elseif focus == cid then
		...
		selfSay('you can enter in Templar area?')
			talk_state = 1

		elseif talk_state == 1 then
			if msgcontains(msg, 'yes') then
if doPlayerRemoveItem(cid,4873,1) == TRUE then
					travel(cid, 1371, 1315, 9)
					selfSay('Let\'s go!')
				else
					selfSay('Sorry, you no have 1 Templar Pass.')
				end
 			end
			talk_state = 0
